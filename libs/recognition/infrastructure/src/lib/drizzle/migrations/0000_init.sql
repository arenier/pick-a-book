CREATE TABLE "shelf_scans" (
	...
	CONSTRAINT "shelf_scans_status_check" CHECK ("shelf_scans"."status" in ('pending', 'completed', 'failed')),
	CONSTRAINT "shelf_scans_detected_books_check" CHECK (("shelf_scans"."status" = 'completed') = ("shelf_scans"."detected_books" is not null))
);
